...
class QuizQuestionFonetica : AppCompatActivity(), View.OnClickListener {

    private lateinit var animationView: LottieAnimationView

    private var myCurrentPos: Int = 1
    private var myQuestionsListLetra: ArrayList<QuestionFonetica>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myQuestionsListLetra = Constants.getQuestionsLetraMuda()
    ...
    private fun setQuestion() {
        val question = myQuestionsListLetra!![myCurrentPos - 1]
        ...
        if (myCurrentPos == myQuestionsListLetra!!.size) {
        ...
        when (v?.id) {
            ...
            R.id.submitButton -> {
                if (mySelectedPos == 0) {
                    ...
                    when {
                        myCurrentPos <= myQuestionsListLetra!!.size ->{
                            setQuestion()
                        }
                        else -> {
                            ...
                            intent.putExtra(Constants.TOTAL_QUESTIONS, myQuestionsListLetra!!.size)
                            startActivity(intent)
                            finish()
                        }
                    }
                } else {
                    val question = myQuestionsListLetra?.get(myCurrentPos - 1)
